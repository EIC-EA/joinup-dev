...
class JoinupContext extends RawDrupalContext {
  ...
  /**
   * Navigates to the delete form of a node entity.
   *
   * @param string $bundle
   *   The type of the node entity.
   * @param string $title
   *   The title of the node.
   *
   * @When I go to the :bundle (content ) :title delete screen
   * @When I visit the :bundle (content ) :title delete screen
   *
   * @throws \Drupal\Core\Entity\EntityMalformedException
   */
  public function visitNodeDeleteForm($bundle, $title) {
    /** @var \Drupal\node\NodeInterface $node */
    $node = $this->getNodeByTitle($title, $bundle);
    $this->visitPath($node->toUrl('delete-form')->toString());
  }
  ...
  protected static function getNodeFieldNameFromAlias($type, $alias) {
    $aliases = [
      ...
      'pledge' => [
        'contact' => 'field_pledge_contact',
        'description' => 'body',
        'owner' => 'field_pledge_owner',
        'state' => 'field_state',
        'type of contribution' => 'field_pledge_contribution_type',
      ],
      'shared' => [
...
JS;
    $found_tags = $this->getSession()
      ->getDriver()
      ->evaluateScript($has_tags_condition);
  ...
  /**
   * Fills in form fields with provided table within a multi property field.
   *
   * Example: When I fill in the following in the "Contact information" field
   *  | Name  | bruceWayne |
   *  | Email | test@example.com |
   *
   * @When I fill in the following in the :field field:
   */
  public function fillFieldsWithinField(string $field, TableNode $fields): void {
    $page = $this->getSession()->getPage();
    $element = $page->findField($field);
    if (empty($element)) {
      // If the field was not found with the default selector, try to find a
      // fieldset as most of the times, if a field contains multiple properties,
      // it is displayed as a fieldset of fields.
      $xpath = "//fieldset[contains(@class, 'form-item') and ./legend[contains(normalize-space(), '{$field}')]]";
      $element = $page->find('xpath', $xpath);
      if (empty($element)) {
        throw new \InvalidArgumentException("Field '{$field}' was not found in the page.");
      }
    }
