...
Feature: Collections Overview
  ...
  Scenario: Users are able to filter collections they have created or that are featured site-wide.
    ...
    When I click "challenges"
    Then the "My challenges content" inline facet should allow selecting the following values:
      | Featured collechallengesctions (2) |
      | My challenges (1)       |
    And the page should be cacheable

    When I click "My challenges" in the "My collections content" inline facet
    Then I should see the following tiles in the correct order:
      | Fed up meatlovers |
    And the "My challenges content" inline facet should allow selecting the following values:
      | Featured challenges (2) |
    ...
    When I am an anonymous user
    And I click "challenges"
    # The anonymous user has no access to the "My challenges" facet entry.
    Then the "My challenges content" inline facet should allow selecting the following values:
      | Featured challenges (2) |
    And the page should be cacheable
